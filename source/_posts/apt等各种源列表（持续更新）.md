...
**因为`https://get.docker.com`已经上不去了，所以该方式作废**

```shell
curl -fsSL https://get.docker.com -o get-docker.sh && sudo DOWNLOAD_URL=https://mirrors.ustc.edu.cn/docker-ce sh get-docker.sh
```

下面提供`get-docker.sh`的代码内容，直接`vim get-docker.sh`然后`chmod +x get-docker.sh`即可，然后使用`DOWNLOAD_URL=https://mirrors.ustc.edu.cn/docker-ce sh get-docker.sh`即可通过中科大源完成安装，代码如下：
